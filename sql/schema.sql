...
CREATE DATABASE IF NOT EXISTS traceq
...
-- sequences 
CREATE SEQUENCE IF NOT EXISTS seq_user     START WITH 1 INCREMENT BY 1 MINVALUE 1;
CREATE SEQUENCE IF NOT EXISTS seq_lost     START WITH 1 INCREMENT BY 1 MINVALUE 1;
CREATE SEQUENCE IF NOT EXISTS seq_found    START WITH 1 INCREMENT BY 1 MINVALUE 1;
CREATE SEQUENCE IF NOT EXISTS seq_claim    START WITH 1 INCREMENT BY 1 MINVALUE 1;
CREATE SEQUENCE IF NOT EXISTS seq_handover START WITH 1 INCREMENT BY 1 MINVALUE 1;

-- triggers
DROP TRIGGER IF EXISTS trg_user_customid;
CREATE TRIGGER trg_user_customid
BEFORE INSERT ON user_data
FOR EACH ROW
SET NEW.custom_id = IF(
  NEW.custom_id IS NULL OR NEW.custom_id='',
  CONCAT('U', DATE_FORMAT(UTC_TIMESTAMP(), '%y%m'), UPPER(HEX(NEXT VALUE FOR seq_user))),
  NEW.custom_id
);

DROP TRIGGER IF EXISTS trg_lost_customid;
CREATE TRIGGER trg_lost_customid
BEFORE INSERT ON lost_reports
FOR EACH ROW
SET NEW.custom_id = IF(
  NEW.custom_id IS NULL OR NEW.custom_id='',
  CONCAT('L', DATE_FORMAT(UTC_TIMESTAMP(), '%y%m'), UPPER(HEX(NEXT VALUE FOR seq_lost))),
  NEW.custom_id
);

DROP TRIGGER IF EXISTS trg_found_customid;
CREATE TRIGGER trg_found_customid
BEFORE INSERT ON found_reports
FOR EACH ROW
SET NEW.custom_id = IF(
  NEW.custom_id IS NULL OR NEW.custom_id='',
  CONCAT('F', DATE_FORMAT(UTC_TIMESTAMP(), '%y%m'), UPPER(HEX(NEXT VALUE FOR seq_found))),
  NEW.custom_id
);

DROP TRIGGER IF EXISTS trg_claims_customid;
CREATE TRIGGER trg_claims_customid
BEFORE INSERT ON claims
FOR EACH ROW
SET NEW.custom_id = IF(
  NEW.custom_id IS NULL OR NEW.custom_id='',
  CONCAT('C', DATE_FORMAT(UTC_TIMESTAMP(), '%y%m'), UPPER(HEX(NEXT VALUE FOR seq_claim))),
  NEW.custom_id
);

DROP TRIGGER IF EXISTS trg_handover_customid;
CREATE TRIGGER trg_handover_customid
BEFORE INSERT ON handover_logs
FOR EACH ROW
SET NEW.custom_id = IF(
  NEW.custom_id IS NULL OR NEW.custom_id='',
  CONCAT('H', DATE_FORMAT(UTC_TIMESTAMP(), '%y%m'), UPPER(HEX(NEXT VALUE FOR seq_handover))),
  NEW.custom_id
);